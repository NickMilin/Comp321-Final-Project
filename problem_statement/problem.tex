...
\noindent Initially, the highway is boring, with a "Scenic Value" of $0$ for every section. However, before the tourist season begins, the Department of Transportation completes a series of $M$ beautification projects. Each project targets an inclusive range of highway $[L, R]$ and increases the Scenic Value of every section in that range by an integer $V$.\\
...
\noindent Unfortunately, because of the new beautification projects, an increased number of people have started driving on the highway. Due to this influx in traffic, accidents may occur at specific sections. An accident acts as a complete roadblock. You cannot drive through a section that has an accident, nor can you start or end your trip exactly at that index. \\

\noindent \textbf{Note:} Entry and exit onto the highway are only allowed at the indices strictly before or strictly after an accident. \\

\noindent \textbf{The Trip Rule:}
You must choose a single contiguous segment of the highway to drive on. This segment must not contain any accidents. Your goal is to maximize the \textbf{Total Scenic Value}, the sum of Scenic Values of the sections included in your trip.\\

\noindent The input provides the details of the beautification projects, followed by $Q$ independent traffic scenarios. For each scenario, you must calculate the maximum Total Scenic Value possible.
...
The first line contains two integers $N$ and $M$ ($1 \le N, M \le 2\,000\,000$), representing the length of the highway and the number of beautification projects. \\

\noindent The next $M$ lines describe the projects. Each line contains three integers $L, R, V$:
\begin{itemize}
    \item The range $[L, R]$ ($1 \le L \le R \le N$) has its Scenic Value increased by $V$ ($-1\,000\,000 \le V \le 1\,000\,000$).
\end{itemize}
...
\begin{itemize}
    \item The line contains $K$ ($1 \le K \le 10$) distinct integers $a_1, a_2,\dots, a_K$  ($1 \le a_i \le N$), representing the locations of the accidents.
\end{itemize}
